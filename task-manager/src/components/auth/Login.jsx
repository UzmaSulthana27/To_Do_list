...
import { login, authStorage } from "../../api/api";
    ...
    <div className="bg-white p-6 rounded shadow max-w-md mx-auto">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          <button disabled={loading} type="submit" className="px-4 py-2 bg-blue-600 text-white rounded">{loading ? "Please wait..." : "Login"}</button>
          <button type="button" onClick={switchToRegister} className="text-sm text-gray-600">Create account</button>
        </div>
      </form>
    </div>
  );
